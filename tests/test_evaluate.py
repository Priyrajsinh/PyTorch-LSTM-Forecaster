...
def _make_checkpoint(tmp_path: Path, config: dict[str, Any]) -> Path:
    """Save a synthetic checkpoint and return its path."""
    import joblib
...
def test_inverse_scale_roundtrip() -> None:
    ...
    original = rng.standard_normal(50).astype(np.float32) * 5 + 10  # fake °C values
    # Scale the values manually
    dummy = np.zeros((50, 3), dtype=np.float32)
    dummy[:, 1] = original
    scaled = scaler.transform(dummy)[:, 1]
    recovered = _inverse_scale(scaled, scaler, n_features=3, target_idx=1)
    np.testing.assert_allclose(recovered, original, atol=1e-4)


def test_inverse_scale_output_shape() -> None:
    """_inverse_scale output has same length as input."""
    scaler = _make_scaler(n_features=3)
...
def test_evaluate_multi_horizon_returns_correct_keys(
    ...
    """evaluate_multi_horizon returns a dict with one key per horizon."""
    cfg = _mini_config()
...
def test_plot_forecast_creates_png(tmp_path: Path, monkeypatch: Any) -> None:
    """plot_forecast saves a PNG to reports/figures/forecast_vs_actual.png."""
    cfg = _mini_config()
    _write_processed(tmp_path)
    _make_checkpoint(tmp_path, cfg)
    monkeypatch.chdir(tmp_path)
